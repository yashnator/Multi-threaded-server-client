...
void *client_thread(void* td_args) { 
    ...
    while(true){
        ...
        buffer[cnt_bytes] = '\0';
        string server_reply = string(buffer);
        // cout << server_reply << endl;
        if(server_reply == busy_reply){
            wait_for_millisec(Taloha);
            continue;
        } else if(server_reply != idle_reply){
            ...
            for(int i = 0; i < pkt_str.length(); ++i){
                if(pkt_str[i] == ',' || pkt_str[i] == '\n'){
                    if(curr_word == "HUH!"){
                        ...
                        int sleep_mult = get_random(backoff_counter);
                        --sleep_mult;
                        if(backoff_counter < 30) backoff_counter <<= 1;
                        cout << "Shleep for " << sleep_mult * Taloha << endl;
                        wait_for_millisec(sleep_mult * Taloha);
                        break;
                    }
...
int main() {
    ...
    cout << "Average time: " << (accumulate(completion_times.begin(), completion_times.end(), 0.0) / (num_threads * 1.0)) << " ms" << endl;
